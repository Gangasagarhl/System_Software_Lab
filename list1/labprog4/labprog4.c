...
void execute(){
	...
	const char *hlgs_filename = "lab4.txt";
	printf("\nNow creating file named 'lab4.txt'\n ");
	
	int hlgs_fd= open(hlgs_filename, O_RDWR);
	if(hlgs_fd ==-1){
	perror("Open  mistake:\n");
	exit(EXIT_FAILURE);
	}
...
printf("\n\n\nThe content of the directory after doing operation:\n");
system("ls -l");
